#input string copied in:

input_1 = """51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
def calibrator(input):
    lines = input.splitlines()  # Use splitlines() to split the input string into lines
    ...
    def combine_digits(digits):
        if len(digits) == 2:
            ...
            raise ValueError("The input list must contain exactly two integers.")
    ...
    for line in lines:
        ...
        for char in chars:
            if char.isnumeric():
                list_of_just_numbers.append(int(char))

        list_of_last_two_digits= []
        list_of_last_two_digits.append(list_of_just_numbers[0])
        list_of_last_two_digits.append(+ list_of_just_numbers[-1])

        line_sum = combine_digits(list_of_last_two_digits)
        total_sum += line_sum
